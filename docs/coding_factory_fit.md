...
| `docs/tasks/` | Per-item plans produced by `/writing-plans`, plus the task-master queue folders below (created on first use) |
| `skills/task-master/SKILL.md` | Canonical task-master behaviour: drafts a codegraph-aware task brief from a `docs/plan.md` item into `docs/tasks/drafts/`; never implements, approves, or queues. `.claude/commands/task-master.md` (Claude) and `AGENTS.md` (Codex/other harnesses) are adapters that point here, not duplicates of it |
| `scripts/factory/task-master.sh` | The deterministic half of task-master: `task-master.sh <item> --lane micro\|standard\|high-risk [--force] [--terms a,b,c] [--db <path>]`. Validates the lane and item, creates `docs/tasks/{drafts,queued,active,done,blocked,failed}/` (each with a `.gitkeep`) idempotently, extracts codegraph query terms from the plan item's backticked paths/identifiers plus `--terms`, calls `codegraph-query.py`, and writes `docs/tasks/drafts/P<item>-<slug>.md` with the frontmatter skeleton (`approval: pending`, `runner_eligible: false`, always) and the `## Codegraph Context`/`## Approval Notes` sections pre-filled (lane-conflict and codegraph-staleness/unavailable warnings). Exit 2 usage error, 3 plan item not found, 4 draft exists without `--force`. Never touches `docs/plan.md` or app code; its only writes are under `docs/tasks/` |
| `scripts/factory/codegraph-query.py` | Read-only Python 3 stdlib queries against `.codegraph/codegraph.db` (`search`, `file`, `symbols`, `related`, `status`); opens read-only (`mode=ro`, falling back to `mode=ro&immutable=1`), never writes to `.codegraph/`, never runs an indexer. `--json` for machine consumption, a tab-separated "human table" by default (what `task-master.sh` actually parses, so neither script depends on `jq`). `status` reports the whole-index staleness verdict: any file's `content_hash` mismatching its on-disk sha256, or the newest `indexed_at` predating `HEAD`'s commit time (read via `git log -1 --format=%ct`) |
| `docs/tasks/{drafts,queued,active,done,blocked,failed}/` | The task queue skeleton task-master creates: `drafts/` holds what task-master writes; a human (or a future runner) moves a draft into `queued/` after approving it; task-master never writes into `queued/` beyond the `.gitkeep` |

Gitignore was changed so `.claude/settings.json`, `.claude/agents/`, and `.claude/commands/` are tracked while `.claude/settings.local.json` and `.factory/` stay local.
...
3. /task-master <n> --lane <lane>  -> docs/tasks/drafts/P<n>-<slug>.md, codegraph-aware, checkbox tasks already in it (skills/task-master/SKILL.md); /writing-plans still used for factory-internal tasks with no plan item, saved under docs/tasks/<n>-<slug>.md as before
...
## Task-master, the queue folders, and future runner eligibility

`skills/task-master/SKILL.md` and `scripts/factory/task-master.sh` draft a codegraph-aware
task brief from a `docs/plan.md` item into `docs/tasks/drafts/P<n>-<slug>.md`. Task-master
never implements, approves, or queues a task; the queue folders
(`docs/tasks/{drafts,queued,active,done,blocked,failed}/`, each with a `.gitkeep`) exist so a
human (or a future runner) has somewhere to move a draft once it *is* approved and queued.

There is no runner yet. When one is built, a task is eligible to run only when all three of
`approval: approved`, `status: queued`, and a valid `lane` hold — `runner_eligible` is always
derived from those three at read time, never a field a human or an LLM hand-sets to `true`
(task-master itself always writes it `false`). The same runner/dispatcher will be the thing
that finally updates `docs/plan.md` at close-out (marking an item Done, Needs Verification,
Blocked, or Failed) for any task whose `plan_item` is not `null` — task-master itself never
touches `docs/plan.md`, and until the runner exists the controller keeps doing that update by
hand, exactly as today. That future runner should also fail a stale-ledger check if a task in
`docs/tasks/done/` references `plan_item: N` but `docs/plan.md` item `N` is not `Done` or
`Needs Verification` — not built in this run, recorded here so the runner task inherits it.
...
- **Run 5 (task-master skill, 2026-09-19):** added a repo-local, harness-neutral drafting skill (`skills/task-master/SKILL.md`) with a deterministic front end (`scripts/factory/task-master.sh`, `scripts/factory/codegraph-query.py`) so Claude (`/task-master`) and Codex (`AGENTS.md`) produce byte-for-byte the same draft skeleton from the same `docs/plan.md` item. Two bugs only showed up once the worked example (item 4) was actually run against the real `.codegraph/codegraph.db`, not from reading the code: (1) `argparse` subparsers that share a `--db`/`--limit`/`--json` dest with the top-level parser silently reset it to the subparser's own default when the flag is given *before* the subcommand name and not repeated after it -- fixed by giving the subparser copies `default=argparse.SUPPRESS` and keeping the real defaults only on the top-level parser; (2) a bash `read -r tag f2 f3 f4 f5` reading a 6-field TSV row silently folds the last two fields (including their separating tab) into the final variable -- always size the `read` var list to the exact field count, one per subcommand's TSV shape, not "however many fields I remember writing." Lesson generalizing (1) and (2): a script that is only exercised through its own selftest's synthetic fixtures can look right and still be wrong against the one real, live input (the actual codegraph db, actually queried) that the worked example is supposed to prove out -- run the worked example for real, read its output, before trusting the selftest cases modeled on it. Also confirmed live in this run: `.codegraph/` is re-indexed by a root-owned background watcher outside the controller's control (file count and `newest_indexed_at` changed between two queries seconds apart with no `codegraph init` run by this session) -- `codegraph-query.py` never triggers it and treats every value as a live read, not a cached fact.
