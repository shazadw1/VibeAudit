...
| `scripts/factory/verify.sh` | `tsc --noEmit` + `next lint` + `vitest run`; writes `.factory/last-verify.json` |
| `scripts/factory/commit-gate.sh` | Blocks `git commit` if a secret-looking file or pattern is staged, or if the last verify was not green (doc-only commits exempt) |
